...
interface TokenPocketWallet extends EventEmitter<TokenPocketWalletEvents> {
    ...
    on(evnet: string, func: Function): Promise<void>;
    off(event: string, func: Function): Promise<void>;
    emit(event: string, func?: Function): any;
}
...
export class TokenPocketWalletAdapter extends BaseMessageSignerWalletAdapter {
    ...
    async connect(): Promise<void> {
        try {
            ...
            if (!wallet.isConnected) {
                // HACK: TokenPocket doesn't reject or emit an event if the popup is closed
    ...
    async disconnect(): Promise<void> {
        ...
        if (wallet) {
            ...
            try {
                ...
                wallet.emit('error', new WalletDisconnectionError(error?.message, error));
            }
    ...
    private _disconnected = () => {
        ...
        if (wallet) {
            ...
            wallet.emit('error', new WalletDisconnectedError());
            wallet.emit('disconnect');
        }
    };
}
